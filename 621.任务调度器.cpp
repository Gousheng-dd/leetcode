...
class Solution {
public:
    //桶思想，设置k（出现次数最多的任务个数）个桶，每个桶大小为n+1保证同一任务恰好不冲突
    int leastInterval(vector<char>& tasks, int n) {
        int m=tasks.size();
        if(!m){
            return 0;
        }
        int *hashtab=new int[26];
        memset(hashtab,0,sizeof(int)*26);
        for(int i=0;i<m;i++){
            hashtab[tasks[i]-'A']++;
        }
        sort(hashtab,hashtab+26,cmp);
        int cnt=1;
        while(cnt<26&&hashtab[cnt]==hashtab[0]){
            cnt++;
        }
        int maxnum=hashtab[0];
        delete []hashtab;
        return max(m,(n+1)*(maxnum-1)+cnt);
    }
private:
